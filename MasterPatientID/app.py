...
import AllergiesJSONResponse as jsonAllergyResponse
import CCDParser as parser
import HTTPRequest as http
import pathlib
import DictionaryMatcher as matcher
    ...
    if request.method=="POST":
        if 'file' in request.files: #If the 'file' form button was clicked
            ...
            for entry in allergymultiselect:
                ...
                split = entry.split(':')
                if (split[1] != ''):
                    allergyIdentifiers["code"].append((rx.lookup(split[1].lower()))) #This will produce a url like ../AllergyIntolerance?display="Hives"&display="Rashes"... Bascially we need to query by code per SIIM rules
            ...
            try:
                ...
                allMatches = matcher.weightedPatientDictionaryMatch(demographics_dict,PatientJSONDicts,allergyMatches)
